...
if __name__ == '__main__':
    ...
    good_scen = create_scen(s1_values, 'Bad1')
        ...
        for _ in range(3):
            convergence_metrics = [
                ArchiveLogger(
                    ...
                    base_filename=f"Multi_MORDM_good_seed_{_}.tar.gz",
                ),
                EpsilonProgress(),
            ]
            result = evaluator.optimize(nfe=25000, searchover='levers',
                                        convergence=convergence_metrics,
                                        epsilons=[1,1,1,1,1,0.1], reference=good_scen,
    ...
    results_epsilon.to_csv('Overijssel MORDM_epsilon_good_scen.csv', index=False)
    merged_archives.to_csv('Overijssel_MORDM_Policies_good_scen.csv', index=False)
